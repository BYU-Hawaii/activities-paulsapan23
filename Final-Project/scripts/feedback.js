document.getElementById('registrationForm').addEventListener('submit', function(event) {
    ...
    var usernameValid = /^[a-zA-Z0-9]{5,}$/.test(name); // Username should be at least 5 characters long and contain only letters and numbers
    ...
    var contactValid = /^(?:\(\d{3}\)|\d{3}-)\d{3}-\d{4}$/.test(contact); // US phone number validation
    ...
    document.getElementById('contactFeedback').style.display = contactValid ? 'none' : 'block';

    document.getElementById('nameFeedback').textContent = nameValid ? '' : 'Username should be at least 5 characters long and contain only letters and numbers.';
    ...
    document.getElementById('contactFeedback').textContent = contactValid ? '' :  'Phone number validation';
